## Map submission

<!-- If this PR adds a map from a submission issue, link that issue here.
For non-map changes, delete the fields and checks that do not apply. -->

- Related submission issue:
- Map name:
- Author:
- Original or archived source:
- Notes about provenance or uncertain metadata:

## Checklist

- [ ] The directory and all three filenames use the same lowercase map ID.
- [ ] The MP file was extracted from the original download without modifying
      its map data, or the ZIP contains required scenario support files.
- [ ] Both images are PNG files; one is a compact preview and one is full-size.
- [ ] I added the entry to `maps.json` in ID order.
- [ ] I ran `python3 tools/validate_archive.py` successfully.
- [ ] The source permits redistribution, and the submission contains no
      executables, unrelated archives, or unrelated content.
